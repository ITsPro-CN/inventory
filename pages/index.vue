		...
		<ClientOnly fallback-tag="span">
			<t-layout class="h-full w-full relative">
				...
				<t-aside id="aside" class="h-full max-sm:absolute max-sm:!w-screen max-sm:z-100 sm:border-r-1 border-gray-400 shrink-0 flex flex-col" width="324px" v-if="showMenu">
						...
						<t-tree
							...
							<template #label="{node}">
								<div class="w-full flex justify-between items-center">
									<div>{{ node.label }}</div>
									<t-icon name="share" v-if="node.data.share_user && node.data.share_user.length"></t-icon>
									<t-icon class="animate-pulse !text-red-500" name="folder-move" v-if="movingEntity.has(node.data)"></t-icon>
									<div class="text-gray-400" v-if="node.data.items.length">{{ node.data.items.length }}</div>
								</div>
							</template>
							<template #operations="{node}">
								<t-space class="select-none" size="small" @click.stop="">
									<t-dropdown trigger="click" maxColumnWidth="200px">
										<div class="pl-2"><t-icon name="more"></t-icon></div>
										<t-dropdown-menu>
					...
					<div class="border-t-1 border-gray-400 p-2">
						<t-row :gutter="[4, 4]">
				...
				<t-content class="relative h-full flex flex-col">
					<div class="w-full p-2 border-b-1 border-gray-400 flex items-center">
						...
						<t-breadcrumb class="flex-1 overflow-x-scroll" :max-item-width="'150'">
							<template v-if="currentLocationID && entity[currentLocationID].owner">
								<t-breadcrumb-item @click="currentLocationID = ''" v-if="entity[currentLocationID].owner === user.sub">
									<template #icon><t-icon name="home" /></template>
									{{ user.name || user.username || "我" }}的库存
								</t-breadcrumb-item>
								<t-breadcrumb-item @click="currentLocationID = '_share'" v-else>
									<template #icon><t-icon name="folder-shared" /></template>
									共享
								</t-breadcrumb-item>
							</template>
...
<script setup lang="ts">
	...
	const reservedFolder: (EntityData & {id: string; name: string; filter: (e: EntityType) => boolean})[] = [
		{
			id: "",
			name: `${user.name || user.username || "我"}的库存`,
			image: "user",
			active: true,
			filter: (e: EntityType) => !e.parent && e.owner === user.sub && (!e.is_container || config.$state.isDisplaySubContainer),
		},
		{
			id: "_share",
			name: "共享",
			image: "folder-shared",
			active: true,
			filter: (e: EntityType) => e.owner !== user.sub && (!e.parent || !entity.value[e.parent]) && (!e.is_container || config.$state.isDisplaySubContainer),
		},
		...
		{id: "_myshare", name: "已共享", image: "folder-shared", filter: (e: EntityType) => Boolean(e.owner === user.sub && e.share_user && e.share_user.length > 0)},
	];
	...
	const updateLocationTree = () => {
		const folders = Object.values(entity.value).filter((e) => e.is_container && e.active && e.owner);
		tree_location.value = [
			{...entity.value[""], children: folders.filter((e) => e.owner === user.sub && !e.parent)},
			{...entity.value["_share"], children: folders.filter((e) => e.owner !== user.sub && (!e.parent || !entity.value[e.parent]))},
		];
	};
	if (import.meta.client) {
		const entity_raw = await $fetch<EntityData[]>("/api/entity", {
			method: "GET",
			retry: false,
		});
		entity.value = {
			...Object.fromEntries(entity_raw?.map((i) => [i.id, new Entity(i)]) ?? []),
		...
		Object.values(entity.value).forEach((e) => {
			e.generateRelation(entity.value, user.sub);
		});
		updateLocationTree();
	}
	...
	const onEntityUpdate = async (formData: EntityData & {propArray?: [string, any][]}) => {
		try {
			...
			if (formData.id) {
				...
				entity.value[data.id].generateRelation(entity.value, user.sub);
	...
	const isAllowDrop = ({dragNode, dropNode, dropPosition}: {dragNode: TreeNodeModel; dropNode: TreeNodeModel; dropPosition: number}) => {
		if (dragNode.data.id === "" || dragNode.data.id === "_share") return false;
		if (dropNode.data.id === "") return dropPosition === 0;
		if (dropNode.data.id === "_share") return false;
		return true;
	};
	const onTreeDrag: TreeProps["onDragEnd"] = async ({node}) => {
		const parent: TreeNodeModel = tree.value.getParent(node.data.id);
		if (!parent || (node.data.parent ?? "") === parent.data.id) return;
		try {
	...
	const moveEntity = async (entityToMove: EntityType[], to: string | undefined) => {
		...
		entityToMove.forEach((e: EntityType) => {
			...
			entity.value[e.id].generateRelation(entity.value, user.sub);
		});
		updateLocationTree();
	};
</script>
